/* =============================================================================
   ...
   Market : Spain
   Source : PANEL_F
   Target : SILVER_DWH.FACT_ES_PANEL_F
   Notes  : Product key derived from category description (hashed). Three lookups
            applied: channel, sales type, units (measure flag). Output kept at fact
            grain (keys + measure); descriptive attributes live in the product master.
   Engine : Snowflake
   ============================================================================= */

INSERT INTO SILVER_DWH.FACT_ES_PANEL_F
...
WITH cte_base AS (
    SELECT
        s.*,
        ROW_NUMBER() OVER (
            PARTITION BY MKT_DSC, GEO_DSC, CAT_DSC, PER_DSC, KPI_FLG
            ORDER BY s.LOA_DAT DESC
        ) AS rn
    FROM BRONZE_STG.STG_ES_PANEL_F s
    WHERE s.LOA_DAT > COALESCE(
        (SELECT MAX(LOA_DAT) FROM SILVER_DWH.FACT_ES_PANEL_F),
        DATE '1900-01-01')
),
cte_keys AS (
    SELECT
        b.*,
        UPPER(TRIM(CAT_DSC))                  AS PDT_PCK_N,
        SHA2(UPPER(TRIM(CAT_DSC)), 256)       AS PDT_COD_N,
        'PANEL_F'                             AS CHL_ORIGEN_N,
        'PANEL_F'                             AS SALTYPE_ORIGEN_N,
...
cte_map_chl AS (
    ...
    FROM SILVER_DWH.MAP_ES_CHANNEL    WHERE SRC_DSC = 'PANEL_F'
),
cte_map_st AS (
    SELECT UPPER(TRIM(SAL_TYP_SRC)) AS ORIGEN_N, SAL_TYP_COD
    FROM SILVER_DWH.MAP_ES_SALES_TYPE WHERE SRC_DSC = 'PANEL_F'
),
cte_map_unt AS (
    SELECT UPPER(TRIM(KPI_FLG_SRC)) AS ORIGEN_N, KPI_FLG_COD
    FROM SILVER_DWH.MAP_ES_UNITS      WHERE SRC_DSC = 'PANEL_F'
)
SELECT
