/* =============================================================================
   ...
   Market : Spain
   Source : PANEL_I  (sell-in)
   Target : SILVER_DWH.FACT_ES_PANEL_I
   Notes  : Wider business key (includes OTC classification levels).
   Engine : Snowflake
   ============================================================================= */

INSERT INTO SILVER_DWH.FACT_ES_PANEL_I
...
WITH cte_base AS (
    SELECT
        s.*,
        ROW_NUMBER() OVER (
            PARTITION BY CNY_DSC, PDT_DSC, PDT_PCK, CAT_DSC,
                         OTC_1_DSC, OTC_2_DSC, OTC_3_DSC, OTC_4_DSC,
                         KPI_FLG, PER_DSC
            ORDER BY s.LOA_DAT DESC
        ) AS rn
    FROM BRONZE_STG.STG_ES_PANEL_I s
    WHERE s.LOA_DAT > COALESCE(
        (SELECT MAX(LOA_DAT) FROM SILVER_DWH.FACT_ES_PANEL_I),
        DATE '1900-01-01')
),
cte_keys AS (
    SELECT
        ...
        'PANEL_I'                       AS CHL_ORIGEN_N,
        'PANEL_I'                       AS SALTYPE_ORIGEN_N,
...
cte_map_chl AS (
    ...
    FROM SILVER_DWH.MAP_ES_CHANNEL    WHERE SRC_DSC = 'PANEL_I'
),
cte_map_st AS (
    SELECT UPPER(TRIM(SAL_TYP_SRC)) AS ORIGEN_N, SAL_TYP_COD
    FROM SILVER_DWH.MAP_ES_SALES_TYPE WHERE SRC_DSC = 'PANEL_I'
),
cte_map_unt AS (
    SELECT UPPER(TRIM(KPI_FLG_SRC)) AS ORIGEN_N, KPI_FLG_COD
    FROM SILVER_DWH.MAP_ES_UNITS      WHERE SRC_DSC = 'PANEL_I'
)
SELECT
